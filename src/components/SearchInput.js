...
function SearchInput({classname,placeholder="Search by keyword",...props}) {
  return (
    <div className={`border-[1px] border-gray-400 w-full max-w-screen-sm rounded-full mt-5 px-5 py-3  flex flex-row items-center gap-4 ${classname}`}>
        ...
        <input className={` outline-none text-lg  placeholder:text-gray-500 truncate `} {...props} placeholder={placeholder} />
    </div>
  )
}
